    ...
    <div class="shadow dark:bg-gray-600/50 w-3/4 p-4 rounded-lg">
        ...
        <table class="table table-fixed table-auto w-full border-collapse border border-2 border-slate-500">
            ...
            <tr class="w-48">
                ...
                <th class="border-collapse border border-slate-500 ">scenarios</th>
            </tr>
            </thead>
            <tbody>
            @foreach($apps as $app)
                <tr class="table-row" wire:key="{{$app->id}}">
                    ...
                    <td class="table-cell text-center border border-slate-700">{{$app->user->name}}
                        / {{$app->user->email}}</td>
                    <td class="table-cell text-center border border-slate-700">{{$app->created_at}}</td>
                    <td class="table-cell text-center text-green-600 border border-slate-700"><a href="{{route('app.scenarios',['app'=>$app->id])}}">test scenarios</a></td>
                </tr>
            @endforeach
            </tbody>
        </table>
