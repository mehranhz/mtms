<div class="flex justify-center w-full py-4 dark:text-indigo-200/50">
    <div class="shadow dark:bg-gray-600/50 w-3/4 p-4 rounded-lg">
        @if($showCreateForm)
            <form action="{{route('permission.store')}}" method="post" class="mx-6">
                @csrf
        ...
        @else
            <button class="rounded-lg bg-indigo-600 mb-4 px-4 py-2 text-white "
                    wire:click="activateForm">{{__('add new permission')}}</button>
        @endif
        @if($showUpdateForm)
            <form action="{{route('permission.update',['permission'=>$activePermission->id])}}" method="post" class="mx-6">
                @csrf
                @method('PUT')
                <div class="my-2">
                    <label for="title" class="text-gray-100">{{__('title')}}</label>
                    <input
                        class="w-full p-2 rounded-lg text-gray-900 focus:outline focus:outline-offset-0 focus:outline-4 outline-violet-600"
                        name="title"
                        placeholder="{{__('title')}}" required value="{{$activePermission->title}}"/>
                </div>
                <div class="my-2">
                    <label for="description" class="text-gray-100">{{__('description')}}</label>
                    <textarea name="description" id="" cols="30" rows="10"
                              class="w-full p-2 rounded-lg text-gray-900 focus:outline focus:outline-offset-0 focus:outline-4  focus:outline-violet-600">{{$activePermission->description}}</textarea>
                </div>
                <div class="flex w-full gap-4">
                    <input type="submit" value="{{__('save')}}"
                           class="rounded-lg bg-indigo-600 mb-4 px-4 py-2 text-white">
        ...
        <table
            ...
            @foreach($permissions as $permission)
                <tr class="table-row" wire:key="{{$permission->id}}">
                    <td class="table-cell text-center border border-slate-700">{{$permission->id}}</td>

                    <td class="table-cell text-center border border-slate-700">{{$permission->title}}</td>
                    <td class="table-cell text-center border border-slate-700">{{$permission->description}}</td>
                    <td class="table-cell text-center border border-slate-700">{{$permission->created_at}}</td>
                    <td class="table-cell text-center border border-slate-700">
                        <button
                            class="text-green-400" wire:click="activateUpdateForm({{$permission->id}})">{{__('edit')}}</button>
